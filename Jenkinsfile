            ...
            node(label: 'docker') {
              script {
                try {
                  ...
                  sh '''docker run -i --name="$BUILD_TAG-volto" -e VOLTO=$VOLTO -e NAMESPACE="$NAMESPACE" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" plone/volto-addon-ci:alpha'''
